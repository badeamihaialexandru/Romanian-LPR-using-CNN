...
def validate_plate( candidates):
    ...
    for each_candidate in candidates:
        ...
        if average >= highest_average:
            ...
            plate_type=cca2.plate_type_of_objects[index]
        index+=1

    return plate_type,license_plate

def calculate_plate(local_license_plate,img_num):
...
def split_plate(license_plate):
    ...
    ax1.imshow(first_half, cmap="gray")
    ax2.imshow(second_half, cmap="gray")
    plt.show()
    return first_half,second_half

# The invert was done so as to convert the black pixel to white pixel and vice versa
#license_plate = np.invert(cca2.plate_like_objects[1])
if len(cca2.plate_like_objects)==0 :
    print("Nu au fost detectate placute in imagine !")
    exit(0);

license_type,license_plate=validate_plate(cca2.plate_like_objects)
...
if license_type==1:
    ...
    first_half_f=first_half
    second_half_f=second_half
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2)
    ax1.imshow(labelled_first_half, cmap="gray")
    ax2.imshow(labelled_second_half, cmap="gray")
    calculate_plate(labelled_first_half,1)
    calculate_plate(labelled_second_half,2)
...
calculate_plate(labelled_plate,0)
plt.show()
